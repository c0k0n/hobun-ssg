...
| Cloudflare Pages | — | hosting; the Git integration runs the build and serves `dist/` |
...
3. `cpSync(public, dist)` — the only static assets: the favicon and `_headers`.
...
- **Static (production + preview):** `/404` is a normal route, so `toSSG` writes `dist/404.html`, and Pages serves it for any unmatched path (Pages auto-detects a `404.html` and uses it as the custom 404 page — this is documented behavior, confirmed against Cloudflare's docs).
